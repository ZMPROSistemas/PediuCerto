									...
									<div class="card-body py-0 px-2 m-0">
										<form class="my-0 pt-0">
                			                <div class="form-row align-items-center" >
												<div class="col-2">
													<input type="text" value="" class="form-control form-control-sm" id="buscaCliente" ng-model="buscaCliente" placeholder="Procura Rápida">
												</div>
			...
			echo $dados_empresa["em_fanta"];
										...
										<!--div class="col-auto">
											<div class="input-group-btn">
												...
												<div class="col-auto">
													<md-button class="btnPesquisar pull-left" style="border: none;" ng-click="ContasRecebidas(dataI | date : 'yyyy-MM-dd', dataF | date : 'yyyy-MM-dd', empresa, cliente, canc)" style="color: white;">
										...
										<div class="table-responsive p-0 mb-2" style="overflow: hidden;">
											<table class="table table-sm table-striped" style="background-color: #FFFFFFFF; color: black;">
												...
												<tbody>
													<tr dir-paginate="contas in contasRecebidos | filter:{ct_nome:buscaCliente} | orderBy:'-ct_pagto' | itemsPerPage:20">
...
<?php }?>
			...
			var restcontasRecebidos = function (dataI, dataF, empresafilial){
				$scope.contasRecebidos = '';
				$scope.arrayNull = true;
				if (dataI == undefined) {
					dataI = $scope.dataI;
				...
				$http({
					method:'GET',
					url: $scope.urlBase+'contas.php?receber=S&listaContaRecFast=S&token=<?=$token?>&us_id=<?=$us_id?>&empresa_matriz=<?=$empresa?>&empresa_filial='+empresafilial+'&dataI='+dataI+'&dataF='+dataF+'&quitado=S'
				}).then(function onSuccess(response){
			...
			$scope.ContasRecebidas = function (dataI, dataF, empresafilial){

				restcontasRecebidos(dataI, dataF, empresafilial);
			}
