...
$hora = date('H:i:s');
    ...
    if (isset($_GET['receber'])) {
        ...
        if(isset($_GET['listaContaRecFast'])){
            ...
            $lista = '{"result":[' . json_encode(listaContasRecebidas($conexao, $empresa, $dataI, $dataF, $quitado)). ']}';
            echo $lista;
        ...
        if(isset($_GET['listaContaReceberFast'])){
            ...
            $lista = '{"result":[' . json_encode(listaContaReceberFast($conexao, $empresa, $vencida, $quitado)). ']}';
            echo $lista;
        ...
        if(isset($_GET['totalContaReceberFast'])){
            ...
            $lista = '{"result":[' . json_encode(totalContaReceberFast($conexao, $empresa, $quitado)). ']}';
            echo $lista;
    ...
    function listaContasPagas($conexao, $empresa, $dataI, $dataF, $quitado){
        ...
        (select dc_sigla from tipo_docto where dc_codigo = ct_tipdoc and dc_empr = ct_empresa) as ct_tipdoc, ct_valorpago, ct_canc, ct_vencto, ct_parc, ct_nome  
    ...
    function listaContasRecebidas($conexao, $empresa, $dataI, $dataF, $quitado){
        ...
        FROM contas where $empresa and ct_receber_pagar = 'R' and ct_pagto is not null and ct_canc <> 'S' and ct_quitado = 'S'
        and ct_pagto between '$dataI' and '$dataF';";
    ...
    function listaContaReceberFast($conexao, $empresa, $vencidas, $quitado){
        ...
        FROM contas where $empresa $vencidas and ct_receber_pagar = 'R' and ct_quitado = 'N' order by ct_vencto;";
    ...
    function totalContaReceberFast($conexao, $empresa, $quitado){
        
        $retorno = array();

        $sql = "select (select sum(ct_valor) FROM contas where ct_vencto < current_date() and $empresa and ct_receber_pagar = 'R' and ct_quitado = 'N') as vencidas,
        (select sum(ct_valor) FROM contas where ct_vencto = current_date() and $empresa and ct_receber_pagar = 'R' and ct_quitado = 'N') as vencendo,
        (select sum(ct_valor) FROM contas where ct_vencto > current_date() and $empresa and ct_receber_pagar = 'R' and ct_quitado = 'N') as a_vencer;";
