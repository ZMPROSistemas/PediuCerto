...
if (isset($_GET['dadosProdutosSimplificado'])) {
	...
	if ($searchGrupo != "") {
		$andGrupo = " AND pd_grupo LIKE '%" . $searchGrupo . "%'";
	} else {
		$andGrupo = "";
	}

	if ($searchSubGrupo != "") {
		$andSubGrupo = " AND pd_subgrupo LIKE '%" . $searchSubGrupo . "%'";
	} else {
		$andSubGrupo = "";
	}

	$lista = json_encode(dadosProdutosSimplificado($conexao, $empresaMatriz, $andNome, $andGrupo, $andSubGrupo, $offset, $pagesize));
	echo $lista;
...
function dadosProdutosSimplificado($conexao, $empresaMatriz, $andNome, $andGrupo, $andSubGrupo, $offset, $pagesize) {


	$sql = "SELECT COUNT(*) AS count FROM produtos where pd_matriz = $empresaMatriz and pd_deletado != 'S' $andNome $andGrupo $andSubGrupo;";
	...
	$query = "select t1.pd_id, t1.pd_cod, t1.pd_desc, t1.pd_empresa, t1.pd_marca, t1.pd_vista, t1.pd_prazo, t2.sbp_descricao as pd_subgrupo
	from produtos as t1 inner join subgrupo_prod as t2 on t1.pd_subgrupo = t2.sbp_codigo and t1.pd_empresa = t2.sbp_empresa 
	where t1.pd_deletado != 'S' and t1.pd_matriz = $empresaMatriz $andNome $andGrupo $andSubGrupo ORDER BY pd_desc LIMIT $offset, $pagesize;";
	...
	while ($row = mysqli_fetch_assoc($result)) {
		$retorno[] = array(
			...
			'pd_subgrupo' => ucwords(utf8_encode($row['pd_subgrupo'])),
			'pd_marca'=> ucwords(utf8_encode($row['pd_marca'])),
