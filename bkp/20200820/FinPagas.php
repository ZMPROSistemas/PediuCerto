...
<style>
	...
	echo $dados_empresa["em_fanta"];
								    	...
								    	<div class="col-auto">
											<div class="input-group-btn">
												<button type="button" class="btn btn-outline-dark btn-sm" ng-click="ContasPagas(dataI | date : 'yyyy-MM-dd', dataF | date : 'yyyy-MM-dd', empresa, cliente, canc)" style="color: white;">
													<i class="fas fa fa-search" ></i>
												</button>
											</div>
								    	</div>
								...
								<table class="table table-sm table-striped" style="background-color: #FFFFFFFF; color: black;">
									<thead class="thead-dark">
										<tr style="font-size: 1em !important;">
											...
											<th scope="col" style=" font-weight: normal; text-align: left;" ng-click="ordenar('ct_nome')">Nome Cliente</th>
									...
									<tbody>
										<tr dir-paginate="contasRecebidos in contasRecebidos | orderBy:'-ct_pagto' | itemsPerPage:10">
											...
											<td ng-bind="contasRecebidos.dc_sigla" align="center"></td>
										</tr>
									</tbody>
								</table>
							</div>
							...
							<div class="container col-12 p-2" style="border:none; background-color: #999999FF; color: black;">
								<div class="row align-items-center">
									<div class="col-3" style="text-align: left;">
										...
										<span style="color: black;">Total Pago: <b class="col-auto">{{totalcontasRecebidos[0].ct_valorpago | currency: 'R$ '}}</b></span>
...
<?php }?>
		   ...
		   $scope.sortKey = function(keyname){
			...
			var restTotalcontasPagas = function(dataI,dataF,empresafilial, cliente, canc){
				$http({
					method:'GET',
			...
			var restcontasPagas = function (dataI,dataF,empresafilial, cliente, canc){
				if (dataI == undefined) {
					dataI = '';
				}
				if (dataF == undefined) {
				...
				$http({
					...
					restTotalcontasPagas(dataI,dataF,empresafilial, cliente, canc);
				}).catch(function onError(response){
			...
			$scope.ContasPagas = function (dataI,dataF,empresafilial, cliente, canc){

				restcontasPagas(dataI,dataF,empresafilial, cliente, canc);
			}
