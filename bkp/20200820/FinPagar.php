...
<style>
	...
	<?php if (base64_decode($empresa_acesso) == 0) {?>
										...
										<div class="col-auto">
											<select class="form-control form-control-sm" id="empresa" ng-model="empresa">
												...
												<?php foreach ($empresaLista as $empresaLista){ ?>
													<option value="<?=$empresaLista['em_cod']?>" ><?=$empresaLista['em_fanta']?></option>
													<?php } ?>
											</select>

											<!--select class="form-control form-control-sm" id="empresa" ng-model="empresa">
												<option value="">Todas as Empresas</option>
												<option ng-repeat="emp in dadosEmpresa" value="{{emp.em_cod}}" ng-selected="emp.em_cod = <?=base64_decode($empresa)?>">{{emp.em_fanta}} </option>
											</select-->
										</div>
	<?php } else {
	echo $dados_empresa["em_fanta"];
										...
										<div class="col-auto">
											<input type="text" class="form-control form-control-sm" ng-model="pDocto" placeholder="Pesquisar por Documento">
										</div>
								    	...
								    	<div class="col-auto">
											<div class="input-group-btn">
												<button type="button" class="btn btn-outline-dark btn-sm" ng-click="contasPagarLista(dataI, dataF, empresa, cliente, canc)" style="color: white;">
						    ...
						    <div class="table-responsive px-0" style="overflow: hidden;">
								<table class="table table-sm table-striped pb-0" style="background-color: #FFFFFFFF; color: black; cursor: pointer;">
									...
									<tbody>
										...
										<tr dir-paginate="contasPagar in contasPagar | filter:{ct_empresa:empresa} | filter:{ct_cliente_forn:cliente} | filter:{ct_canc:canc} | filter:{ct_docto:pDocto} | orderBy:'sortKey' | itemsPerPage:10" ng-class="contasPagar.ct_vencta == 'Vencido' ? 'venc' : contasPagar.ct_vencta == 'Hoje' ? 'vencH':''">
...
<?php } ?>
<?php if (substr($me_compraA_Pagar, 3,1) == 'S') {?>
														<a class="dropdown-item" ng-click="buscarCont(contasPagar,'E')">Editar</a>
<?php } ?>
											...
											<div class="col-6">
												...
												<div>
													...
													<md-switch md-invert ng-model="statusCheck" class="fa-pull-right" ng-change="verCaixa(statusCheck)" style="margin-right:28px;">
														<span style="color:#000;">Lançar baixa no caixa</span>
													</md-switch>
												</div>
...
?>
    ...
    <script  id="INLINE_PEN_JS_ID">
		...
		angular.module("ZMPro",['ngMessages','ngMatchMedia','ngMaterial','md.data.table','angularUtils.directives.dirPagination','money-mask']);
...
<?php }?>
		   ...
		   $scope.sortKey = function(keyname){
		    ...
		    $scope.AdicionarDespesa = function (despesa, ct_emissao) {

				enviarDespesa(despesa, ct_emissao);

		    	function enviarDespesa(despesa, ct_emissao){
					...
					$http({
						...
						method: 'POST',
						 headers: {
				          ...
				          url: $scope.urlBase+'SalvaDespesa.php?SalvarDespesa=S&us_id=<?=$us_id?>&e=<?=$empresa?>&eA=<?=$empresa_acesso?>&cx=false&bc_codigo=null&ct_emissao='+ct_emissao
			...
			$scope.editarDispesa = function(parc, mudarVencimento, mudarValor, mudarTipoDocto, descricao, fornecedor){
				//alert();
				if (parc == "") {
				...
				$http({
						...
						method: 'POST',
						 headers: {
						  ...
						  url: $scope.urlBase+'SalvaDespesa.php?editarDispesa=S&us_id=<?=$us_id?>&e=<?=$empresa?>&eA=<?=$empresa_acesso?>&cx=false&bc_codigo=null&ct_emissao='+ct_emissao
