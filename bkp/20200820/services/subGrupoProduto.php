<?php
    ...
    if (isset($_GET['dadosSubGrupo'])) {
        ...
        if (isset($_GET['lista'])) {
            ...
            $lista = '{"result":[' . json_encode(listaSubGrupoProduto($conexao, $empresaMatriz)). ']}';
            echo $lista;
        }
        ...
        if (isset($_GET['editar'])) {
           ...
           $imageSub = $_GET['imageSub'];
           $lancarSite = $_GET['lancarSite'];
           $DestacarSite = $_GET['DestacarSite'];


            $editar = editarGrupoProduto($conexao, $empresaMatriz, $sbp_id, $sbp_desc, $sbp_Grup , $imageSub, $lancarSite, $DestacarSite);
        }
        ...
        if (isset($_GET['excluir'])) {
            ...
            $excluir = excluirGrupoProduto($conexao, $empresaMatriz, $sbp_id);
        }
    }

    function listaSubGrupoProduto($conexao, $empresaMatriz){
        $resultado = array();

        $sql = "SELECT sbp_id, sbp_codigo, sbp_empresa, (select em_fanta from empresas where em_cod = sbp_empresa) as pe_empresa,
        sbp_matriz, sbp_codigo, sbp_descricao , sbp_grupo, sbp_lanca_site, sbp_destaca_site,
        (select grp_descricao from grupo_prod where sbp_grupo = grp_codigo and grp_matriz = sbp_matriz and grp_empresa = sbp_empresa) as sbp_grupo_desc, sbp_imagem
        FROM zmpro.subgrupo_prod where sbp_matriz = $empresaMatriz and sbp_deletado <> 'S';";
        ...
        while ($row = mysqli_fetch_assoc($query)) {
            array_push($resultado, array(
                ...
                'pe_empresa' => utf8_encode(ucwords(strtolower($row['pe_empresa']))),
    ...
    function subGrupoProduto($conexao, $empresaMatriz, $sub_id){
        ...
        FROM zmpro.subgrupo_prod where sbp_matriz = $empresaMatriz and sbp_id = $sub_id;";
    ...
    function editarGrupoProduto($conexao, $empresaMatriz, $sbp_id, $sbp_desc, $sbp_Grup, $imageSub, $lancarSite, $DestacarSite){
    ...
    function excluirGrupoProduto($conexao, $empresaMatriz, $sbp_id){
        $retorno = array();
