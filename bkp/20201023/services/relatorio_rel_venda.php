...
if (isset($_GET['status'])) {
	...
	if ($status == 'A') {
		...
		$vd_status = 'and vd_status = "Fechado" and vd_emis >= "' . $dataI . '" and vd_emis <= "' . $dataF . '"';
	} else if ($status == 'D') {
		$vd_status = 'and vd_status = "Devolvido" and vd_emis >= "' . $dataI . '" and vd_emis <= "' . $dataF . '"';
	}
...
if (isset($_GET['relatorio'])) {
	...
	if (isset($_GET['totaisVendas'])) {
		$lista = '{"result":[' . json_encode(totaisVendas($conexao, $empresaMatriz, $vd_empr, $vdi_empr, $dataI, $dataF)) . ']}';
		echo $lista;
	}
...
function itensVenda($conexao, $empresaMatriz, $vd_empr, $dataI, $dataF, $vd_id) {
	...
	$sql = "SELECT vdi_prod, vdi_descricao, vdi_quant, vdi_preco, vdi_valprod, vdi_valdescto, vdi_total
			from venda_item
	...
	while ($row = mysqli_fetch_assoc($query)) {
		array_push($resultado, array(
			...
			'vdi_descricao' => ucwords(utf8_encode($row['vdi_descricao'])),
			'vdi_quant' => $row['vdi_quant'],
			'vdi_valprod' => utf8_encode($row['vdi_valprod']),
			'vdi_valdescto' => utf8_encode($row['vdi_valdescto']),
...
function totaisVendas($conexao, $empresaMatriz, $vd_empr, $vdi_empr, $dataI, $dataF) {
...
(select sum(vdi_custo) from venda_item where vdi_canc <> 'S' and vdi_emis between '$dataI' and '$dataF'
and vdi_matriz = $empresaMatriz $vdi_empr) as custo
