...
if (isset($_GET['clientes'])) {
	...
	if ($clientes == null) {
		...
		$vd_cli = '  and vd_nome like "%' . $clientes .'%"';
	}
} else {
	$vd_cli = '';
}
...
if (isset($_GET['funcionario'])) {
	...
	if ($funcionario == null) {
		...
		$vd_func = ' and vd_func = (select pe_cod from pessoas where pe_nome like "%' . $funcionario .'%" and pe_empresa = ' . $empresa . ' and pe_vendedor = "S")' ;
	}
} else {
	$vd_func = '';
}
...
function relatorio($conexao, $empresaMatriz, $dataI, $dataF, $vd_empr, $vd_cli, $vd_func, $vd_forma) {
	...
	$sql = "SELECT vd_doc, vd_cli, vd_func, (select pe_nome from pessoas where pe_cod = vd_func and pe_empresa = vd_empr and pe_vendedor = 'S') as nome_func, 
	vd_empr, (select em_fanta from empresas where em_cod = vd_empr) as em_fanta, 
	vd_matriz, vd_emis, vd_valor, vd_desc, vd_total, vd_vl_pagto_dn, vd_vl_pagto_ca , vd_vl_pagto_bl, vd_vl_pagto_dp, vd_nome 
	FROM vendas where vd_emis between '$dataI' and '$dataF' and vd_canc<>'S' and vd_pgr<>'D' and vd_canc<>'S' and vd_pgr<>'D'
	and vd_matriz = $empresaMatriz $vd_empr $vd_cli $vd_func $vd_forma;";
	...
	*/
	while ($row = mysqli_fetch_assoc($query)) {
		array_push($resultado, array(
			...
			'vd_cli' => $row['vd_cli'], 
			'vd_func' => $row['vd_func'], 
			'nome_func' => utf8_encode($row['nome_func']), 
			'vd_empr' => $row['vd_empr'], 
			...
			'tipo_docto' => verificaTipoDoc($row['vd_vl_pagto_dn'], $row['vd_vl_pagto_ca'], $row['vd_vl_pagto_bl']),
...
function relatorioTotalGrupoBy($conexao, $empresaMatriz, $dataI, $dataF, $vd_empr, $vd_cli, $vd_func, $vd_forma) {
    ...
    sum(vd_total) as total FROM vendas where vd_emis between '$dataI' and '$dataF' and vd_canc<>'S' and vd_pgr<>'D' and vd_canc<>'S' and vd_pgr<>'D'
	and vd_matriz = $empresaMatriz $vd_empr $vd_cli $vd_func $vd_forma;";
...
function verificaTipoDoc($tip_dinheiro, $tip_cartao, $tip_boleto){

	$retorno =''; 
	if($tip_dinheiro <> '0.00'){
		$retorno = 'DN';
	}else if($tip_cartao <> '0.00'){
		$retorno = 'CT';
	}else if($tip_boleto <> '0.00'){
		$retorno = 'BL';
	}else{
		$retorno = 'PR';
	}
