...
//	$ct_empresa = $empresaAcesso;
	...
	foreach ($parcelas as $parcelas) {
		...
		adicionarCompra ($conexao, $despesa['ct_empresa'], $ct_matriz, $despesa['ct_docto'], $despesa['ct_cliente_forn'], $despesa['ct_obs'], $ct_emissao, $parcelas['vencimento'], $parcelas['parcela'], $parcelas['vezes'], $ct_tipdoc, $ct_quitado, $despesa['ct_historico'], $ct_pagto, $ct_valorpago, $selecionarCaixa, $bc_codigo, $i);
		$i++;
	};
}
...
function adicionarCompra($conexao, $ct_empresa, $ct_matriz, $ct_docto, $ct_cliente_forn, $ct_obs, $ct_emissao, $vencimento, $parcela, $vezes, $ct_tipdoc, $ct_quitado, $ct_historico, $ct_pagto, $ct_valorpago, $selecionarCaixa, $bc_codigo, $i) {

	$retorno = array();

	$sql = "insert into contas(ct_empresa, ct_matriz, ct_docto, ct_cliente_forn, ct_nome, ct_obs, ct_emissao, ct_vencto, ct_valor, ct_parc, ct_canc, ct_tipdoc, ct_receber_pagar, ct_quitado, ct_historico, ct_desc_hist, ct_pagto, ct_valorpago) values (
	$ct_empresa, $ct_matriz, $ct_docto, (select pe_cod from pessoas where pe_id = $ct_cliente_forn), (select pe_nome from pessoas where pe_id = $ct_cliente_forn), UPPER ('$ct_obs'), '$ct_emissao', '$vencimento', '$parcela', '$vezes', 'N', '$ct_tipdoc', 'P', '$ct_quitado', (select ht_cod from historico where ht_id = $ct_historico), (select ht_descricao from historico where ht_id = $ct_historico),  '$ct_pagto', '$ct_valorpago');";
