		...
		list-style-type: none;
			...
			<div ng-controller="ZMProCtrl">	
									...
									<div class="card-body py-0 px-2 m-0">
    			                        <form class="my-0 pt-0">
                			                <div class="form-row align-items-center">
												...
												<div class="col-2">
													<input type="text" value="" class="form-control form-control-sm" id="buscaCliente" ng-model="buscaCliente" placeholder="Todos os Fornecedores">
												</div>
										...
										<div class="table-responsive p-0 mb-0" style="overflow: hidden;">
											<table class="table table-sm table-striped" style="background-color: #FFFFFFFF; color: black;">
												...
												<tbody style="line-height: 2em;">
													<tr dir-paginate="fornecedor in dadosFornecedor | orderBy:'sortKey':reverse | itemsPerPage:20">
														...
														EM 04/11/2020, APÓS CONVERSA COM MICHEL, FOI DECIDIDO NÃO PERMITIR ALTERAÇÃO NEM EXCLUSÃO DE FORNECEDORES
														PARA NÃO DAR PROBLEMAS EM OUTROS LUGARES
				...
				<?php if (substr($me_empresa, 2, 1) == 'S') {?>
																		<md-button ng-click="editarFornecedor(fornecedor.pe_id)">Editar</md-button>
					<?php } ?>
																	</md-menu-item>
																	<md-menu-item>
				<?php if (substr($me_empresa, 3, 1) == 'S') {?>
																	<md-button ng-click="excluirFornecedor(fornecedor.pe_id, fornecedor.pe_nome)">Excluir</md-button>
					<?php }?>
																	</md-menu-item>
																</md-menu-content>   
															</md-menu>
											...
											<div class="form-row align-items-center">
												<div class="col-3" style="text-align: left;">
													<div class="row justify-content-start">
														<span style="color: gray;">Registros: <b>{{dadosFornecedor.length}}</b></span>
													</div>
												</div>
											</div>
										</div>	
...
<!--										<div class="form-group col-md-4 col-12">
										...
										<div class="form-group col-md-4 col-7" ng-init="fornecedor.pe_cidade = fornecedor[0].pe_cidade">
											...
											<input type="text" class="form-control form-control-sm capoTexto campoObrigatorio" id="pe_cidade" ng-model="fornecedor.pe_cidade" ng-value="fornecedor.pe_cidade" autocomplete="off" onKeyUp="tabenter(event,getElementById('pe_uf'))">
										</div>
...
<?php }?>
			...
			var dadosFornecedor = function () {
				...
				$http({
					method: 'GET',
					url: $scope.urlBase+'ConsultaClienteFornecedor.php?dadosCliente=S&us_id=<?=$us_id?>&e=<?=$empresa?>&eA=<?=$empresa_acesso?>&cliente_fornecedor='+$scope.cliente_fornecedor+'&situacao='+$scope.situacao+'&ativo='+$scope.ativo+'&dados=S&empresa='+$scope.empresa+'&cliente='+$scope.nomeFornecedor+'&cidade='+$scope.cidade
				}).then(function onSuccess(response){
			...
			$scope.busca = function(empresa, buscaCliente, buscaCidade) {
				...
				$scope.nomeFornecedor = buscaCliente;
