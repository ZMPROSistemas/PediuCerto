<?php
   ...
   function pedidos($conexao, $user, $token){
        ...
        $sql = "SELECT * FROM zmpro.pedido_food where ped_empresa = (select em_cod from empresas where em_token='$token') and ped_cliente_cod = $user order by ped_emis desc, ped_hora_entrada desc
        ";
   ...
   function pedidos_item($conexao , $idPedido){
        ...
        $sql = "SELECT pdi_id, pdi_emis, pdi_produto, pdi_descricao, pdi_quantidade, pdi_total, pdi_obs, pdi_status
         FROM pedido_item_food where pdi_idprim = $idPedido;";
