...
if (!isset($_SESSION['arrayProdutos'])){
	...
	$_SESSION['arrayProdutos'] = buscarListaProdutos($conexao, base64_decode($empresa));
	//echo "entrou";

}

$cacheProdInfo = buscarCacheProdutos($conexao,base64_decode($empresa), base64_decode($empresa_acesso));
...
//$_session['arrayProdutos']=1552;



function buscarListaProdutos($conexao, $empresa) {

	$retorno = array();

	$sql = "SELECT pd_id, pd_cod, pd_un, pd_desc, pd_empresa, pd_subgrupo, pd_custo, pd_vista from produtos where pd_matriz = $empresa;";
	...
	while ($row = mysqli_fetch_assoc($query)) {
		array_push($retorno, array(
			...
			'pd_un'=>$row['pd_un'],
			'pd_desc' => utf8_decode($row['pd_desc']),
...
function buscarCacheProdutos($conexao, $matriz, $empresa) {
	if ($empresa == 0) {
		$empresa = $matriz;
	}
	...
	if ($row['ca_produto'] == null) {
		...
		$_SESSION['arrayProdutos'] = buscarListaProdutos($conexao, $empresa);
	}
									...
									<form id="formProd">
										<div class="form-row">
											<div class="form-group col-10">
												...
												<input autofocus type="text" class="form-control form-control-sm" id="codProd" autocomplete="off" ng-model="formProd.codProd" placeholder="Informe o Código do Produto" onKeyUp="tabenter(event,getElementById('btnIncluir'))" ng-blur="pesquisaProdutoDesktop(formProd.codProd, formProd.qtde)" onkeypress='return event.charCode >= 48 && event.charCode <= 57' required>
											</div>
											<div class="form-group col-2">
												<label>Quantidade</label>
												<div class="input-group" ng-init="formProd.qtde  = 1">
													<input type="text" class="form-control form-control-sm" id="qtde" autocomplete="off" ng-value="formProd.qtde" ng-model="formProd.qtde" onKeyUp="tabenter(event,getElementById('btnIncluir'))" onfocus="this.focus();this.select()" ng-blur="pesquisaProdutoDesktop(formProd.codProd, formProd.qtde)" onkeypress='return event.charCode >= 48 && event.charCode <= 57' ng-disabled="true">
...
?>															
								...
								<div class="card-footer p-2">
									<div class="form-row align-items-center">
										<div class="col-6">
											<button class="btn btn-outline-danger" style="color: white;" ng-if="produtoLista[0]"  ng-click="sairLancamento()"><i class="fas fa-window-close"></i> Cancelar</button>
...
?>															
    ...
    <script  id="INLINE_PEN_JS_ID">
		...
		angular.module("ZMPro",['ngMessages','ngMatchMedia','ngMaterial','angularUtils.directives.dirPagination','swxSessionStorage','swxLocalStorage']);
	    angular.module("ZMPro").controller("ZMProCtrl", function ($scope, $http, $timeout, $mdSidenav, $log, $sessionStorage, $localStorage) {
			...
			var atualizaProdutoLista = function(){
				var cache = $sessionStorage.get('produtoLista');

				angular.forEach(cache, function(value, key){
					$scope.produtoLista.push(cache[key]);
				});
			}
			atualizaProdutoLista();
...
<?php }?>
			...
			$scope.pesquisaProduto = function(pd_cod, empresa){
				...
				if(pd_cod != ''){
					...
					if (pesquisaArray != undefined) {
						...
						if ($scope.produto != ''){
							adicionaProdutoArray($scope.produto.pd_cod, $scope.produto.pd_desc, $scope.produto.pd_un, $scope.produto.pd_custo, $scope.produto.pd_vista, $scope.produto.pd_subgrupo);
							//alert("funcao");
						}
					} else {
			...
			$scope.pesquisaProdutoDesktop = function(pd_cod, pd_qtde){
				//alert($scope.empresa);
			...
			var adicionaProdutoArray = function (pd_cod, pd_desc, pd_un, pd_custo, pd_vista, pd_subgrupo) {
				...
				$sessionStorage.remove('produtoLista');
				$sessionStorage.put('produtoLista', $scope.produtoLista);
				$scope.qtde = 1;
			}
