<?php
    ...
    function buscaDados($pdo, $dados, $desc, $ID){
        ...
        if ($passou=='T'){
            if ($desc!=''){
                $sql="select * from usuarios where us_empresa=".$dados['cod_empresa']." and (us_nome LIKE '%$desc%') order by us_nome";
            }else{
                if ($ID!=''){
                    $sql="select * from usuarios where us_empresa=".$dados['cod_empresa']." and us_id=$ID order by us_nome";    
                }else{
                    $sql="select * from usuarios where us_empresa=".$dados['cod_empresa']." order by us_nome";
                }
            }
            ...
            foreach ($pdo->query($sql) as $row) {
                array_push($resultado, array(
                    ...
                    'nivel' => $row['us_nivel'],
                    'ativo' => $row['us_ativo'],
                    'tela_inicio' => $row['us_tela_inicio'],
                    'tela_config' => $row['us_tela_config'],
                    'tela_usuarios' => $row['us_tela_usuarios'],
                    'tela_transacoes' => $row['us_tela_transacoes'],
                    'tela_saque' => $row['us_tela_saque']
                ));
            }
        }
        return $resultado;
