<?php
    ...
    function buscaDados($pdo, $dados, $desc, $ID){
        ...
        if ($passou=='T'){
            if ($desc!=''){
                $sql="select * from view_clientes where cl_empresa=".$dados['cod_empresa']." and (cl_nome LIKE '%$desc%') order by cl_nome";
            }else{
                if ($ID!=''){
                    $sql="select * from view_clientes where cl_empresa=".$dados['cod_empresa']." and cl_id=$ID order by cl_nome";    
                }else{
                    $sql="select * from view_clientes where cl_empresa=".$dados['cod_empresa']." order by cl_nome";
                }
            }
            foreach ($pdo->query($sql) as $row) {
                array_push($resultado, array(
                    ...
                    'genero' => $row['cl_genero'],
                    'cep' => $row['cl_cep'],
                    'end' => $row['cl_end'],
                    'end_num' => $row['cl_end_num'],
                    'bairro' => $row['cl_bairro'],
                    'cidade' => $row['cl_cid'],
                    'uf' => $row['cl_uf'],
