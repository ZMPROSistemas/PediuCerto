<?php
    ...
    function buscaDados($pdo, $dados){
        ...
        if ($passou=='T'){
            $sql = "select us_id, us_empresa, us_nome, us_ativo, us_nivel, us_depto from usuarios where us_email=:email and us_senha=:senha";
            ...
            if ($pedido->rowCount()== 0 ){
                ...
                $retorno='{"status":"OK","empresa":"'.$row[0]['us_empresa'].'","usuario":"'.$row[0]['us_id'].'","nome":"'.$row[0]['us_nome'].'","ativo":"'.$row[0]['us_ativo'].'","cod_depto":"'.$row[0]['us_depto'].'","nome_depto":"'.$nome_depto.'","perfil":'.$teste.'}';
            }
        }
        return $retorno;
    }
    ...
    function buscaDepto($pdo,$depto,$empresa){
        $sql = "select dp_nome from departamentos where dp_cod=:cod and dp_empresa=:empresa";
        $pedido =$pdo->prepare($sql);
        $pedido->bindValue(":cod", $depto);
