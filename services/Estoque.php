...
if (isset($_GET['dadosTransferencias'])) {
	...
	if (isset($_GET['empresaDestino'])) {
		$emp = $_GET['empresaDestino'];
		if ($emp == null || $emp == '') {
			$empresaDestino = '';
		} else {
			$empresaDestino = ' and tr_entrada = (SELECT em_cod_local from empresas where em_cod = '.$emp . ')';
		}
	}

	if (isset($_GET['empresaOrigem'])) {
		$empresa = $_GET['empresaOrigem'];
		if ($empresa == null || $empresa == '') {
			$empresaSaida = '';
		} else {
			$empresaSaida = ' and tr_saida = (SELECT em_cod_local from empresas where em_cod = '.$empresa . ')';
		}
	}
	...
	if (isset($_GET['item'])) {
		...
		if ($item == null || $item == '') {
			...
			$tri_prod = ' and tr_id in (SELECT tri_idprim from transf_item where tri_prod = '.$item . ')';
		}
	}

	$lista = '{"result":[' . json_encode(dadosTransferencias($conexao, $empresaMatriz, $empresaSaida, $empresaDestino, $dataI, $dataF, $tri_prod)) . ']}';
	echo $lista;
...
if (isset($_GET['movimento_cardex'])){
	$es_prod = $_GET['pd_cod'];
	$es_empr = $_GET['es_empr'];
	$lista = '{"result":[' . json_encode(movimentacao_cardex($conexao, $es_empr, $es_prod)).']}';
	echo $lista;
}

function dadosTransferencias($conexao, $empresaMatriz, $empresaSaida, $empresaDestino, $dataI, $dataF, $tri_prod) {
	...
	$sql = "SELECT 	tr_id, 
					...
					tr_data, 
					tr_saida, 
					(select UPPER(em_fanta) from empresas where em_cod_local = tr_saida and em_cod_matriz = tr_matriz) as emp_saida, 
					tr_entrada, 
					(select UPPER(em_fanta) from empresas where em_cod_local = tr_entrada and em_cod_matriz = tr_matriz) as emp_entrada, 
					tr_pedido, 
					tr_autorizado, 
					tr_sel, 
					tr_lancado, 
					tr_enviado, 
					tr_recebido 
 			from transferencia where tr_matriz = $empresaMatriz $empresaSaida $empresaDestino $tri_prod
	...
	while ($row = mysqli_fetch_assoc($query)) {
		array_push($retorno, array(
				...
				'tr_saida' => $row['tr_saida'],
				'emp_saida' => utf8_encode($row['emp_saida']),
				'tr_entrada' => $row['tr_entrada'],
				'emp_entrada' => utf8_encode($row['emp_entrada']),
...
function itensTransf($conexao, $empresaMatriz, $dataI, $dataF, $tr_id) {
	...
	while ($row = mysqli_fetch_assoc($query)) {
		...
		array_push($resultado, array(
				...
				'tri_desc' => utf8_encode($row['tri_desc']),
...
function movimentacao_cardex($conexao, $es_empr, $es_prod){
	...
	(SELECT pe_nome FROM pessoas where pe_empresa = sd_empr and pe_cod = sd_usuario and pe_colaborador = 'S') as usuario,
	sd_hora, sd_motivo, sd_valor FROM zmpro.saldo_estoque where sd_empr = $es_empr and sd_prod = $es_prod;";
