...
function dadosNotaEntrada($conexao, $empresaMatriz, $empresa, $dataI, $dataF) {
	...
	while ($row = mysqli_fetch_assoc($resultado)) {
		array_push($retorno, array(
			...
			'cp_forn' => $row['cp_forn'],
			'cp_fnraz' => utf8_encode($row['cp_fnraz']),
...
function dadosNotaSaida($conexao, $empresaMatriz, $empresa, $dataI, $dataF) {
	...
	while ($row = mysqli_fetch_assoc($resultado)) {
		array_push($retorno, array(
					...
					'nf_cli' => $row['nf_cli'],
					'nf_clraz' => utf8_encode($row['nf_clraz']),
					'nf_es' => $row['nf_es'],
...
function itensNotaEntrada($conexao, $empresaMatriz, $dataI, $dataF, $cp_id) {
	...
	while ($row = mysqli_fetch_assoc($query)) {
		...
		array_push($resultado, array(
				...
				'cpi_descricao' => utf8_encode($row['cpi_descricao']),
...
function itensNotaSaida($conexao, $empresaMatriz, $dataI, $dataF, $nf_id) {
	...
	while ($row = mysqli_fetch_assoc($query)) {
		...
		array_push($resultado, array(
				...
				'nfi_desc' => utf8_encode($row['nfi_desc']),
				'nfi_quant' => $row['nfi_quant'],
				'nfi_valor' => utf8_encode($row['nfi_valor']),
...
function dadosFormaPagto($conexao, $empresaMatriz, $empresa, $dataI, $dataF) {
	...
	$sql = "SELECT 	distinct(dc_codigo), dc_sigla,
					dc_descricao
			from tipo_docto
	...
	while ($row = mysqli_fetch_assoc($query)) {
		...
		array_push($resultado, array(
				...
				'dc_sigla' => ucwords(utf8_encode($row['dc_sigla'])),
...
function CaixasAbertos($conexao, $empresaMatriz, $empresa, $funcionario) {
	...
	$sql = "SELECT 	bc_codigo, bc_cod_func,
			bc_descricao
			from bancos
