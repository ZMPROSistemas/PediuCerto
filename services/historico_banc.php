...
/*
    ALTERADO POR KLEYTHON EM 19/08/2020
    Inserir ht_custo_fixo nos selects, etc.
*/
...
$hora = date('H:i:s');
    ...
    if (isset($_GET['historico'])) {
        ...
        if (isset($_GET['salvar'])) {
            ...
            $ht_dc = $_GET['ht_dc'];
            $ht_custo_fixo = $_GET['ht_custo_fixo'];
            if ($ht_custo_fixo == 'true') {
                $ht_custo_fixo = 'S';
            } else {
                $ht_custo_fixo = 'N';
            }

            salvarHistóricoBancario($conexao, $empresaMatriz, $empresaAcesso, $token, $ht_descricao, $ht_dc, $ht_custo_fixo, $data, $hora, $ip, $us_id);
        }
        ...
        if (isset($_GET['editar'])) {
            ...
            editarHistóricoBancario($conexao, $empresaMatriz, $empresaAcesso, $token, $ht_descricao, $ht_dc, $ht_custo_fixo, $ht_id, $data, $hora, $ip, $us_id);
        }
    ...
    function dadosHistóricoBancario($conexao, $empresaMatriz, $empresaAcesso, $token){
        ...
        while ($row = mysqli_fetch_assoc($query)) {
            array_push($retorno, array(
                ...
                'ht_custo_fixo' => verifica($row['ht_custo_fixo']),
            ));
...
function históricoBancario($conexao, $empresaMatriz, $empresaAcesso, $token, $grp_id){
    ...
    ht_cod, ht_descricao, ht_grupo, ht_tipogrup, ht_dc, ht_ordem, ht_centro_custo, ht_desc_centrocusto, ht_custo_fixo
    ...
    while ($row = mysqli_fetch_assoc($query)) {
        array_push($retorno, array(
            ...
            'ht_cod' =>$row['ht_cod'],
            'ht_descricao' => ucwords(strtolower(utf8_encode($row['ht_descricao']))),
            'ht_grupo' =>$row['ht_grupo'],
            'ht_tipogrup' =>$row['ht_tipogrup'],
            'ht_dc' =>utf8_encode($row['ht_dc']),
            'ht_ordem' =>$row['ht_ordem'],
            'ht_centro_custo' =>$row['ht_centro_custo'],
            'ht_desc_centrocusto' => ucwords(strtolower(utf8_encode($row['ht_desc_centrocusto']))),
            'ht_custo_fixo' => verifica($row['ht_custo_fixo']),
        ));
...
function salvarHistóricoBancario($conexao, $empresaMatriz, $empresaAcesso, $token, $ht_descricao, $ht_dc, $ht_custo_fixo, $data, $hora, $ip, $us_id){
    
    $sql = "INSERT INTO historico (ht_cod, ht_empresa, ht_matriz,  ht_descricao, ht_grupo, ht_tipogrup, ht_dc, ht_ordem, ht_centro_custo, 
    ht_desc_centrocusto, ht_custo_fixo) select (max(ht_cod)+1), $empresaMatriz, $empresaMatriz, '$ht_descricao', 0, 0,'$ht_dc', 0, 0, 'NENHUM', '$ht_custo_fixo' FROM historico where (SELECT em_cod FROM empresas where em_token = '$token');";
...
function editarHistóricoBancario($conexao, $empresaMatriz, $empresaAcesso, $token, $ht_descricao, $ht_dc, $ht_custo_fixo, $ht_id, $data, $hora, $ip, $us_id){
    $sql = "UPDATE historico SET ht_descricao = '$ht_descricao', ht_dc = '$ht_dc', ht_custo_fixo = '$ht_custo_fixo' where ht_id = $ht_id";
    $query = mysqli_query($conexao, $sql);
    echo $query;
    $retorno = mysqli_affected_rows($conexao);
...
function excluirHistóricoBancario($conexao, $empresaMatriz, $empresaAcesso, $token, $ht_descricao, $ht_id, $data, $hora, $ip, $us_id){
    ...
    $sql = "DELETE FROM historico WHERE ht_id = $ht_id;";
    $query = mysqli_query($conexao,$sql);
