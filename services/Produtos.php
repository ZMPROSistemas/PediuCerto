...
function produtosAplicativo($conexao, $pdo, $id, $token) {
	...
	$sql = "SELECT sbp_id, sbp_codigo, sbp_descricao, sbp_grupo, sbp_tipo from subgrupo_prod where 
...
function buscarProdutosAplicativo($conexao, $pdo, $id, $token, $subGroup) {
	...
	$retorno = array();
		...
		$sql = "SELECT pd_id, pd_cod, pd_desc, pd_vista, pd_subgrupo, pd_destaquesite, pd_foto_url, pd_disk, pd_composicao, pd_atualizast, pd_codgrade, pd_val1  
		from produtos where pd_matriz=(select em_cod from empresas where em_token = '$token')
		and pd_lanca_site='S' and pd_ativo='S' and pd_subgrupo=$subGroup and (pd_deletado is null or pd_deletado <> 'S') order by pd_desc asc;";
