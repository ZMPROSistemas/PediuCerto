
<?php

header('Access-Control-Allow-Origin: *');
header('Access-Control-Allow-Methods: GET, POST');

//$conexao = mysqli_connect(DB_SERVER, DB_USERNAME, DB_PASSWORD, DB_DATABASE);
// Servidor ZMPro
$DB_SERVER = 'db-zmpro.cg5tyrexerwf.us-east-1.rds.amazonaws.com';
$DB_USERNAME = 'root';
$DB_PASSWORD = 'lxmcz2020';
$DB_DATABASE = 'zmpro';
try{    
    $pdo = new PDO("mysql:host=$DB_SERVER;dbname=$DB_DATABASE;charset=utf8", $DB_USERNAME, $DB_PASSWORD);
    
    //$pdo = new PDO("mysql:host=$DB_SERVER;dbname=$DB_DATABASE;charset=utf8", $DB_USERNAME, $DB_PASSWORD, array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8", PDO::ATTR_EMULATE_PREPARES   => false));

    //$pdo = new PDO("mysql:dbname=". $DB_DATABASE . "; host=". $DB_SERVER, $DB_USERNAME, $DB_PASSWORD, array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8")); 
    //echo 'Conexao efetuada com sucesso!';
    //$conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
}catch(PDOException $e) {
    echo 'ERROR: ' . $e->getMessage();
}

// Servidor New ZMPro
$DB_SERVER = 'db-zmpro.cg5tyrexerwf.us-east-1.rds.amazonaws.com';
$DB_USERNAME = 'root';
$DB_PASSWORD = 'lxmcz2020';
$DB_DATABASE = 'Lexus';
try{
    $pdo1 = new PDO("mysql:host=$DB_SERVER;dbname=$DB_DATABASE;charset=utf8", $DB_USERNAME, $DB_PASSWORD);
}catch(PDOException $e) {
    echo 'ERROR: ' . $e->getMessage();
}

$DB_SERVER = 'db-zmpro.cg5tyrexerwf.us-east-1.rds.amazonaws.com';
$DB_USERNAME = 'root';
$DB_PASSWORD = 'lxmcz2020';
$DB_DATABASE = 'zmchat';
try{
    $pdo2 = new PDO("mysql:host=$DB_SERVER;dbname=$DB_DATABASE;charset=utf8", $DB_USERNAME, $DB_PASSWORD);
}catch(PDOException $e) {
    echo 'ERROR: ' . $e->getMessage();
}