...
date_default_timezone_set('America/Bahia');
    ...
    if (isset($_GET['dadosgrupo'])) {
        if (isset($_GET['lista'])) {
            ...
            $lista = '{"result":[' . json_encode(listaGrupoProduto($pdo, $empresaMatriz, $grp_descricao)). ']}';
            echo $lista;
        }
        ...
        if (isset($_GET['editar'])) {
            $grp_id = $_GET['grp_id'];
            $grp_desc = ucwords(strtolower(utf8_decode($_GET['grp_desc'])));
    ...
    function listaGrupoProduto($pdo, $empresaMatriz, $grp_descricao){
        $resultado = array();

        $sql = "SELECT grp_id, grp_codigo, grp_descricao
        FROM grupo_prod where grp_empresa = :empresa $grp_descricao and grp_deletado <> 'S' order by grp_descricao;";
