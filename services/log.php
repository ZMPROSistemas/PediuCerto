...
//include 'conecta.php';
	...
	function logTaxa($pdo, $token, $obs, $data, $hora, $ip, $us, $us_id){
		/*$sql = "INSERT INTO log (lg_data, lg_hora, lg_ip, lg_func_cod, lg_func_nome, lg_hist, lg_empresa, lg_matriz, 
		lg_sistema) VALUES ('$data', '$hora', '$ip', $us_id, '$us', '$obs', 
		(SELECT em_cod FROM empresas where em_token='$token'), 
		(SELECT em_cod_matriz FROM empresas where em_token='$token'), 'N');";
		*/
		
		$sql = "INSERT INTO log (lg_data, lg_hora, lg_ip, lg_func_cod, lg_func_nome, lg_hist, lg_empresa, lg_matriz, 
		lg_sistema) VALUES (?, ?, ?, ?, ?, ?, 
		(SELECT em_cod FROM empresas where em_token=?), 
		(SELECT em_cod_matriz FROM empresas where em_token=?), ?);";

		$stmt= $pdo->prepare($sql);
        $stmt->execute([$data, $hora, $ip, $us_id, $us, $obs, $token,$token,'N']);
		$row = $stmt->rowCount();
