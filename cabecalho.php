...
$em_cid = $dados_empresa['em_cid'];

$em_cod = base64_encode($dados_empresa['em_cod']);
if (!isset($_SESSION['em_cod'])){
	$_SESSION['em_cod'] = base64_decode($em_cod);
}
if ($_SESSION['em_cod'] != base64_decode($em_cod)) {
    $_SESSION['em_cod'] = base64_decode($em_cod);
}
...
$sqlRamo_atv = "SELECT * FROM zmpro.ramos where rm_cod =". base64_decode($em_ramo).";";
$queryRamo_atv = mysqli_query($conexao, $sqlRamo_atv);
$dadosRamo = mysqli_fetch_assoc($queryRamo_atv);
...
$me_grade = $dados_perfil['cadgradest'] . $dados_perfil['cadgradest_i'] . $dados_perfil['cadgradest_a']. $dados_perfil['cadgradest_e'];
$me_grade1 = base64_encode($me_grade);
...
-->
        ...
        <style>
            ...
            html {
                ...
                background-color: <?=$dadosRamo['background_color']?>;
                color: <?=$dadosRamo['cor_texto_bar']?>;
                background-size: 100%;
           }
            ...
            html {
                ...
                background-image: url(<?=$dadosRamo['background_img']?>);
            }

            .breadcrumb-item {
                color: <?=$dadosRamo['cor_titulo_tela']?> !important;
            }
            .breadcrumb-item svg {
                color: <?=$dadosRamo['cor_titulo_tela']?> !important;
            }
            ...
            .md-toolbar-tools {
                background-color: <?=$dadosRamo['nav_bar']?> !important;
            }

            #sidebar{
                background-color: <?=$dadosRamo['side_bar']?> !important;
...
<!--            <div id="dismiss">
            ...
            <div class="sidebar-header" style="text-align: center; background-color: transparent !important;">
               <img class="img-fluid" width="90%" src="<?=$dadosRamo['logo_sistema']?>">
            </div>
                    ...
                    <li>
                        ...
                        ?>
                            <a href="CadClientes.php?u=<?=$usuario1?>&s=<?=$senha1?>">
                                ...
                                <span style="padding-left: 10px !important">Clientes Pediu Certo</span>
                            </a>
                        <?php
                            }
                        ...
                        ?>
                            <a href="ClientesFoods.php?u=<?=$usuario1?>&s=<?=$senha1?>">
                                <i class="fas fa-user-friends"></i>
...
if (substr($me_produtos, 0, 1) == 'S') {?>
                                ...
                                <!--<li><a href="CadProdutos2.php?u=<?=$usuario1?>&s=<?=$senha1?>&me=<?=$me_produtos1?>">Cadastro de Produtos 2</a></li>-->
                            <?php
}?>
<?php if (base64_decode($em_ramo) == 1){?>
                                <li><a href="CadAdicionaisExcecoes.php?u=<?=$usuario1?>&s=<?=$senha1?>&me=<?=$me_produtos1?>">Adicionais</a></li>
...
<?php }?>
                            ...
                            <?php
                            if (substr($me_grade, 0, 1) == 'S' && base64_decode($em_ramo) == 1) {?>                            
                                <li><a href="CadGrade.php?u=<?=$usuario1?>&s=<?=$senha1?>&me=<?=$me_grade1?>">Grades de Produtos</a></li>
                            <?php }?>                            
                            <?php
                            if (base64_decode($em_ramo) == 2) {?>
                        ...
                        if (base64_decode($em_ramo) == 2) { ?>
                            ...
                            <ul class="collapse list-unstyled" id="financeiroSubmenu" data-parent="#acordiao">
                                ...
                                <!--li><a href="FinMovCaixa.php?u=<?=$usuario1?>&s=<?=$senha1?>">Movimentação de Caixa</a></li-->
                                <!--<li><a href="">Bancos</a></li> -->
                                <!--li><a href="">Movimentação Bancária</a></li-->
                                <li><a href="historico_bancario.php?u=<?=$usuario1?>&s=<?=$senha1?>">Cadastro de Históricos</a></li> 
...
if (substr($me_venda, 0, 1) == 'S') {?>
                            ...
                            <li><a href="FatOrdemServico.php?u=<?=$usuario1?>&s=<?=$senha1?>">Ordem de Serviço</a></li>
                            <!--li><a href="#">CFOP</a></li>
                            <li><a href="#">NCM</a></li-->
                        </ul>
<?php
...
<?php if(base64_decode($em_ramo) == 1){ ?>
                                ...
                                <li><a href="regiaoTaxa.php?u=<?=$usuario1?>&s=<?=$senha1?>&me=<?=$me_empresa1?>">Região e Taxa</a></li>
                            <?php
}
}if (base64_decode($em_ramo) == 2) { ?>
...
<?php if (substr($me_ralatorio, 3, 1) == 'S') {?>
                            <li><a href="AdmPercRateio.php?u=<?=$usuario1?>&s=<?=$senha1?>&me=<?=$me_empresa1?>">Percentuais de Rateio</a></li>
<?php 
    }
} if (base64_decode($em_ramo) == 1) {
?>
...
<?php }}?>
<?php if (substr($me_ralatorio, 6, 1) == 'S' && base64_decode($em_ramo) == 1) {?>
...
<?php }?>
<?php if (substr($me_ralatorio, 7, 1) == 'S') {?>
                            <li><a href="relatorioFluxoCaixa.php?u=<?=$usuario1?>&s=<?=$senha1?>&me=<?=$me_empresa1?>">Fluxo de Caixa</a></li>
<?php }}?>

<?php if (base64_decode($em_ramo) == 1) {?>
                        <li><a href="">Relação de Pedidos</a></li>
<?php }?>
                        </ul> 
                    </li>
                </ul>
<?php if (base64_decode($em_cod) == 1) {?>
                <a style="padding-left: 10px !important" href="emp_food.php">
                    <i class="fas fa-key"></i>
                    <span style="padding-left: 10px !important">Abrir Empresa Foods</span>
                </a></p>

                <a style="padding-left: 10px !important" href="token.php">
                    <i class="fas fa-key"></i>
                    <span style="padding-left: 10px !important">Gerar Token</span>
                </a></p>
<?php }?>
        ...
        <div id="content">
            ...
            <div show-on-mobile class="noPrint">
                <md-content>
                    <md-toolbar class="md-hue-2 site-content-toolbar"  layout="row" md-theme="site-toolbar" style="">
