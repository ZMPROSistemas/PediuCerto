<style>
			...
			<div ng-controller="ZMProCtrl">	
		  		<nav aria-label="breadcrumb">
					<ol class="breadcrumb p-0">
						...
						<li class="breadcrumb-item" aria-current="page"><?=$title_nome1?></li>
						<li class="breadcrumb-item active" aria-current="page"><?=$title_nome2?></li>
					</ol>
				</nav>
	  			...
	  			<div class="row" style="font-size: 0.9em !important">
					<div class="col-lg-12 pt-0 px-2">
						<div ng-if="lista">
							...
							<div show-on-desktop>
								<div class="card border-dark" style="background-color:rgba(0,0,0, .65); "> 
									...
									<div class="card-body py-0 px-2 m-0">
                                        ...
                                        <!-- Final - Parte de Filtragem de Dados -->
                                        <!-- Botao de Impressão -->
											...
											<table class="table table-sm table-striped" style="background-color: #FFFFFFFF; color: black;">
												<thead class="thead-dark">
                                                    <tr style="font-size: 1em !important;">
                                                        <?php
                                                        foreach ( $itens_grid as $e ){ ?>
                                                          <th scope="col" style=" font-weight: normal; text-align: <?=$e->alinhamento?>;"><?=$e->cabecalho?></th>    
												...
												<tbody style="line-height: 2em; margin:0px; padding:0px;">
													<tr dir-paginate="prod in dadosProdutos | itemsPerPage: pageSize" total-items="totalItems" current-page="pagination.current"> 
                                                        ...
                                                        foreach ( $itens_grid as $e ){ 
                                                            if ($e->campo!=":"){
                                                                if ($e->mascara==""){?>
                                                                    <td style="text-align: <?=$e->alinhamento?>;" ng-bind="prod.<?=$e->campo?>"></td>
                                                                <?php } else {?>
                                                                    <td style="text-align: <?=$e->alinhamento?>;" ng-bind="prod.<?=$e->campo?> | <?=$e->mascara?>:'<?=$e->prefixo?>'"></td>
                                                                <?php } 
                                                            } else {?>
                                                                <td style="text-align: <?=$e->alinhamento?>;" class="ml-auto m-0 p-0">
                                                                    <md-menu>
                                                                        ...
                                                                        <md-menu-content class="dropdown" >
                                                                            <?php
                                                                            foreach ( $menu_grid as $mg ){ 
                                                                                if ($mg->modo_edicao==""){?>
                                                                                    <md-menu-item >
                                                                                        <md-button ng-click="visualizarProduto(prod, 'V', true)"><?=$mg->item?></md-button>
                                                                                    </md-menu-item>
                                                                                <?php } else {
                            ...
                            <!-- Botao de Inclusao -->
                            <?php if (substr($modo_edicao, 1, 1) == 'S') {?>
    ...
    <script  id="INLINE_PEN_JS_ID">
	    ...
	    angular.module("ZMPro").controller("ZMProCtrl", function ($scope, $http, $timeout, $mdSidenav, $log, $window) {
			...
			$scope.excecao = [];
			$scope.ramo =  <?=base64_decode($em_ramo)?>;
			...
			<?php
				...
				else if(base64_decode($em_ramo) == 2){

			?>
				$scope.abaAdicional = false;
			<?php
				}
			?>
...
/*Apagar 19/11/2020  Amarrado com visualizarProduto
...
*/
			$scope.cadastrarAdicional = function (pd_cod){
				//$("#formExcecao").reset();
				$("#ex_desc").val("");
				$("#ex_tipo").val("");
				$("#ex_valor").val("");
...
/*Apagar 19/11/2020 Amarrado com buscarEstoque
...
*/
			...
			$scope.removerExcecaoProd = function(excecao){
				$http({
					method: 'GET',	
					url: $scope.urlBase+'srvcExcecoes.php?removerExcecaoProd=S&us_id=<?=$us_id?>&e=<?=$empresa?>&eA=<?=$empresa_acesso?>&pde_produto='+$scope.produto[0].pd_cod+'&pde_excecao='+excecao.ex_cod
...
*/
			<?php
				include_once('controller/produto.js');
			?>
...
/*Apagar 19-11-2020 amarrado com AdicionarProduto
			...
			$scope.markup = function(pd_custo, pd_vista, pd_prazo){
				...
				console.log($scope.produto[0].pd_custo + " + " + $scope.produto[0].pd_vista)
				
				var pd_custo = $scope.produto[0].pd_custo.replace("R","");
				pd_custo = pd_custo.replace("$","");
				pd_custo = pd_custo.replace(",",".");

				var pd_vista = $scope.produto[0].pd_vista.replace("R","");
				pd_vista = pd_vista.replace("$","");
				pd_vista = pd_vista.replace(",",".");

				
				if (pd_prazo == undefined || pd_prazo == 0 || pd_prazo == '' || $scope.ramo == 1) {
					pd_prazo = pd_vista;
					$scope.produto[0].pd_prazo = pd_prazo;
				}
