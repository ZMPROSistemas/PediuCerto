...
<div show-on-desktop>-->
		  		...
		  		<nav aria-label="breadcrumb">
					<ol class="breadcrumb p-0">
						...
						<li class="breadcrumb-item active" aria-current="page">Relatório De Contagem De Estoque</li>
	...
	echo $dados_empresa["em_fanta"];
						    	...
						    	<div class="form-group col-md-2 col-10" style="margin-right: -10px !important; margin-left: -34px !important;">
								   <div style="margin-top: 40px;">
									...
									<button class="btn btn-outline-dark" lass="btn btn-outline-dark" style="color: white;" ng-click="busca(empresa, dataI | date: 'yyyy-MM-dd', dataF | date: 'yyyy-MM-dd', subgrupo, estoque)"><i class="fas fa-search"></i></button>
					    		...
					    		<table class="table table-striped table-borderless" style="background-color: #FFFFFFFF; color: black;">
									...
									<thead class="thead-dark">
										<tr style="font-size: 1.1em !important;">
											...
											<th scope="col" style=" font-weight: normal;" ng-click="ordenar('em_fanta')" >Código
											</th>

											<th scope="col" style=" font-weight: normal;" ng-click="ordenar('em_fanta')">Contagem
											</th>

											<th scope="col" style=" font-weight: normal;" ng-click="ordenar('pd_desc')">Descrição
											</th>

											<th scope="col" style=" font-weight: normal; width: 80px;" ng-click="ordenar('pd_un')">UN
											</th>
											<th scope="col" style=" font-weight: normal;" ng-click="ordenar('sbp_descricao')">SubGrupo
											</th>
									...
									<tbody>
										...
										<tr dir-paginate="relatorio in relatorio |itemsPerPage:10 | orderBy:sortKey:reverse ">

<!--
											<tr ng-repeat="relatorio in relatorio">
-->
											<td>{{relatorio.em_fanta | limitTo:20}}{{relatorio.em_fanta.length >= 30 ? '...' : ''}}</td>

											<td>{{relatorio.pd_cod}}</td>
											<td></td>
											<td>{{relatorio.pd_desc}}</td>
											<td>{{relatorio.pd_un}}</td>
											<td>{{relatorio.sbp_descricao}}</td>
...
-->
				...
				<tbody>
					...
					<tr ng-repeat="relatorio in relatorioPrint |  orderBy:sortKey:reverse">
						<td>{{relatorio.em_fanta}}</td>
    ...
    <script  id="INLINE_PEN_JS_ID">
	    ...
	    angular.module("ZMPro").controller("ZMProCtrl", function ($scope, $http, $timeout, $mdSidenav, $mdEditDialog, $log, $q) {
...
<?php }?>
			...
			subgrupo();
			 ...
			 var relatorioRegistro = function (empresa, dataI, dataF,subgrupo) {
				$http({
					method: 'GET',
					//url: 'json/estoque.json'
					url: $scope.urlBase+'relatorio_contagem_estoque.php?relatorio=S&dadosRelatorio=S&us_id=<?=$us_id?>&e=<?=$empresa?>&eA=<?=$empresa_acesso?>&empresa='+empresa+'&subgrupo='+subgrupo+'&dataI='+dataI+'&dataF='+dataF
				}).then(function onSuccess(response){
			...
			var relatorioTotalRegistro = function (empresa, dataI, dataF, subgrupo) {
				$http({
					method: 'GET',
					url: $scope.urlBase+'relatorio_contagem_estoque.php?relatorio=S&relatorioTotalRegistro=S&us_id=<?=$us_id?>&e=<?=$empresa?>&eA=<?=$empresa_acesso?>&empresa='+empresa+'&subgrupo='+subgrupo+'&dataI='+dataI+'&dataF='+dataF
				}).then(function onSuccess(response){
...
<?php }?>
    		...
    		var header = function (data) {
		        ...
		        doc.text("Relatório De Contagem De Estoque", 85, 27);
		        doc.setFontSize(12);
		        doc.setTextColor(40);
    		...
    		doc.autoTable(data1.columns, data1.rows,{
		        ...
		        styles: {halign: 'center', theme: 'grid', fontSize: (number = 7), font: 'helvetica', lineColor: [44, 62, 80], lineWidth: 0.55},
			        ...
			        columnStyles: {
			        	0: {halign: 'center',cellWidth: 60}, //emp
			        	1: {halign: 'center', cellWidth: 50}, //cod
			        	2: {halign: 'right', cellWidth: 60}, //cont
			        	3: {halign: 'left'}, //desc
			        	4: {halign: 'center', cellWidth: 50}, //un
			        	5: {halign: 'left'}}, //subG
