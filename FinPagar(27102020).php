		...
		border:1px dashed gray;
			...
			echo $dados_empresa["em_fanta"];
												...
												<div class="ml-auto m-0 p-0">
													<md-button class="btnPesquisar pull-right" style="border: none;" ng-click="contasPagarLista('','', empresa,'','')" style="color: white;">
														<md-tooltip md-direction="top" md-visible="tooltipVisible">Pesquisar</md-tooltip>
														<i class="fas fa fa-search" ></i> Pesquisar
													</md-button>
													<md-button class="btnSalvar pull-right" style="border: none;" style="color: green;" ng-disabled="!contasPagar[0]" ng-click="print()">
										...
										<div class="table-responsive p-0 mb-2" style="overflow: hidden;">
											<table class="table table-sm table-striped" style="background-color: #FFFFFFFF; color: black; cursor: pointer;">
												...
												<tbody>
                                                    <tr dir-paginate="contasPagar in contasPagar | orderBy:'sortKey' | itemsPerPage:20" ng-class="contasPagar.ct_vencto < dataI ? 'venc' : contasPagar.ct_vencto == dataI ? 'vencH':''">
                                                        <td>{{contasPagar.em_fanta | limitTo:30}}{{contasPagar.em_fanta.length >= 30 ? '...' : ''}}</td>
                                                            ...
                                                            <md-menu>
																...
																<md-menu-content class="dropdown" >
																	...
																	<?php if (substr($me_compraA_Pagar, 3,1) == 'S') {?>
																		<md-button ng-click="buscarCont(contasPagar,'B')">Baixar</md-button>
																	<?php } ?>
																	</md-menu-item>
																	<md-menu-item >
																	<?php if (substr($me_compraA_Pagar, 3,1) == 'S') {?>
																		<md-button ng-click="editConta(contasPagar,'E')">Editar</md-button>
																	<?php } ?>
																	</md-menu-item>
																	<md-menu-item >
																		<md-button ng-click="excluir(contasPagar,1)">Excluir</md-button>
																	</md-menu-item>
																</md-menu-content>
															</md-menu>
														</td>
...
<?php }?>
		   ...
		   $scope.sortKey = function(keyname){
			...
			var restcontasPagar = function (){
				...
				$http({
					method:'GET',
					url: $scope.urlBase+'srvcContaPagar.php?listaContasPagar=S&e=<?=$empresa?>&eA=<?=$empresa_acesso?>&empresa='+$scope.empresa+'&buscaNome='+$scope.buscaNome
				}).then(function onSuccess(response){
			...
			$scope.contasPagarLista = function (empresafilial, cliente){
			...
			$scope.buscarProximoDocto = function(){
				...
				$http({
					method:'GET',
					url: $scope.urlBase+'contas.php?pagar=S&proximoDocto=S&token=<?=$token?>&empresa_matriz=<?=$empresa?>'
				}).then(function onSuccess(response){
			...
			$scope.editConta = function (conta,e){
				$http({
					method:'GET',
					url: $scope.urlBase + 'contas.php?buscarContaPagarID=S&ct_id='+conta.ct_id+'&token=<?=$token?>&empresa_matriz=<?=$empresa?>&us_id=<?=$us_id?>&quitado='+$scope.quitado+'&RecPag=P'
				}).then(function onSuccess(response){
		    ...
		    $scope.AdicionarDespesa = function (despesa, selecionarCaixa, bc_codigo, dataPagto, valorPago) {
		    	...
		    	if (selecionarCaixa == true) {
					...
					if (bc_codigo == null || bc_codigo == undefined || bc_codigo == '') {
						...
						enviarDespesa(despesa, selecionarCaixa, bc_codigo, dataPagto, valorPago);
					}
		    	} else {
					enviarDespesa(despesa, selecionarCaixa, bc_codigo, dataPagto, valorPago);
				}
			...
			function enviarDespesa(despesa, selecionarCaixa, bc_codigo, dataPagto, valorPago){
				...
				$http({
					...
					method: 'POST',
						...
						url: $scope.urlBase+'SalvaDespesa.php?SalvarDespesa=S&us_id=<?=$us_id?>&e=<?=$empresa?>&eA=<?=$empresa_acesso?>&cx='+selecionarCaixa+'&bc_codigo='+btoa(bc_codigo)+'&dataPagto='+btoa(dataPagto)+'&valorPago='+btoa(valorPago)
