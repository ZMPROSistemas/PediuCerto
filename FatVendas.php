				    		...
				    		<form class="">
								<div class="form-row align-items-center">-->
										...
										<?php } else {
											...
											<div class="col-auto">
												<input type="date" class="form-control form-control-sm" id="dataI" ng-model="dataI" value="{{dataI}}" ng-change="pararBuscaAutom()">
											</div>
											<div class="col-auto">
												<label for="dataF">até </label>
											</div>
											<div class="col-auto">
												<input type="date" class="form-control form-control-sm" id="dataF" ng-model="dataF" value="{{dataF}}" ng-change="pararBuscaAutom()">
											</div>
											<div class="col-auto ml-3">
											...
											<div class="ml-auto m-0 ">
												<md-button class="btnPesquisar pull-right" style="border: none;" ng-click="modificaBusca(empresa, dataI | date: 'yyyy-MM-dd', dataF | date: 'yyyy-MM-dd', itensPagina)" style="color: white;">
									...
									<div class="card col-12 p-0 mt-0" style="border:none; background-color: rgba(0,0,0, .15);">
										<div class="row">
											<div class="col-7 pl-0"><span style="color:gainsboro;">Vendas</span></div>
											<div class="col-5 pl-2"><span style="color:gainsboro;">Itens da Venda <b>{{documento}}</b></span></div>
										</div>
									</div>
									<div class="card col-12 p-0" style="border:none; background-color:rgba(0,0,0, .8); ">
										<div class="row ">
											<div class="table-responsive col-7 px-0" style="overflow: hidden; border:none !important; background-color: white;">
												<table class="table table-sm table-striped pb-0" style="background-color: #FFFFFFFF; color: black; cursor: pointer; ">
													<thead class="thead-dark">
														<tr>
															...
															<th scope="col" style="font-weight: normal; text-align: left;" ng-click="ordenar('vd_empr')">Empresa</th>
															<th scope="col" style="font-weight: normal; text-align: right;" ng-click="ordenar('vd_total')">Valor Total</th>
														</tr>
													</thead>
													<tbody>
														<tr dir-paginate="vendas in dadosVendas | orderBy:'sortKey':reverse | itemsPerPage:pageSize" ng-click="ConsultaVenda(vendas)" ng-blur="LimpaItens()" ng-class="vendas.vd_nf != 0 ? 'NF': ''">
															...
															<td style="text-align: left;" ng-bind="vendas.em_fanta"></td>
											...
											<div class="table-responsive col-5 px-0" style="overflow:auto; background-color: white; border-left: 8px rgba(0,0,0, .8) solid;">
												<table class="table table-sm table-striped" style="background-color: white; width: 100%; color: black;">
													<thead class="thead-dark">
														<tr>
									...
									<div class="card-footer p-2" ng-repeat="totais in totaisVendas">
										<div class="form-row align-items-center">
											<div class="col-2 pl-0" style="text-align: left;">
												<div class="row justify-content-start">
													<span style="color: grey;">Registros: <b>{{dadosVendas.length}}</b></span>
												</div>
												<div class="row justify-content-start">
													<span style="color: grey;">Ticket Médio: <b>{{totais.total / dadosVendas.length  | currency:'R$ '}}</b></span>
												</div>
												<div class="row justify-content-start">
													<span style="color: grey;">Custo Médio: <b>{{totais.custo / dadosVendas.length | currency:'R$ '}}</b></span>
												</div>
											</div>
											<div class="col-2" >
												<div class="row justify-content-start">
													<span style="color: gainsboro;">Dinheiro: <b class="col-auto">{{totais.dinheiro | currency:'R$ '}}</b></span>
												</div>
												<div class="row justify-content-start">
													<span style="color: gainsboro;">Cartão: <b class="col-auto">{{totais.cartao | currency:'R$ '}}</b></span>
												</div>
												<div class="row justify-content-start">
													<span style="color: gainsboro;">Prazo: <b class="col-auto">{{totais.prazo | currency:'R$ '}}</b></span>
												</div>
											</div>
											<div class="col-5">
												<div class="row justify-content-start">
													<h5 style="color: white;"><small>Total: </small><b class="col-auto">{{totais.total | currency:'R$ '}}</b></h5> 
												</div>
												<div class="row justify-content-start">
													<h5 style="color: tomato;"><small>Total #: </small><b class="col-auto">{{totais.totalred | currency:'R$ '}}</b></h5>
												</div>
											</div>
											<div class="col-3" style="text-align: right;">
												<span style="color: grey;">Itens: <b>{{itensVenda.length}}</b></span>
											</div>
										</div>
									</div>
								</div>
...
<?php }?>
		    ...
		    var totaisVendas = function () {
				$http({
					method: 'GET',
					url: $scope.urlBase+'relatorio_rel_venda.php?relatorio=S&totaisVendas=S&e=<?=$empresa?>&eA=<?=$empresa_acesso?>&empresa='+$scope.empresa+'&dataI='+$scope.dataI+'&dataF='+$scope.dataF+'&funcionario='+$scope.funcionario
				}).then(function onSuccess(response){
		    ...
		    var relatorioVendas = function () {
				$http({
					method: 'GET',
					url: $scope.urlBase+'relatorio_rel_venda.php?relatorio=S&dadosVendas=S&e=<?=$empresa?>&eA=<?=$empresa_acesso?>&empresa='+$scope.empresa+'&dataI='+$scope.dataI+'&dataF='+$scope.dataF+'&funcionario='+$scope.funcionario
				}).then(function onSuccess(response){
			...
			$scope.modificaBusca = function (empresa,dataI,dataF,itensPagina) {
