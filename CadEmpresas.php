...
-->
							<div show-on-desktop>
								<table class="table table-striped table-dark ">
									<thead >
										<tr style="background-color:#000; color:#fff; font-size: 1.1em !important;">
											...
											<th scope="col" style=" font-weight: normal; text-align: right;">Ação</th>
										</tr>
									</thead>
									<tbody ng-repeat="empresa in dadosEmpresa|filter:buscaRapida|limitTo: query.limit: (query.page -1) * query.limit" >
										<tr style="font-size: 1.3em !important;">
											<th scope="row" ng-bind="empresa.em_cod"></th>
											<td ng-bind="empresa.em_razao"></td>
											<td ng-bind="empresa.em_fanta"></td>
											<td ng-bind="empresa.em_cid"></td>
											<td ng-bind="empresa.em_fone"></td>
											<td>
		...
		<?php if (substr($me_empresa, 3, 1) == 'S') {?>
													   	...
													   	<md-button type="submit" aria-label="{{item.name}}" ng-click="excluirEmpresa(empresa.em_cod, empresa.em_cnpj)" class="md-fab md-raised md-mini" style="background-color:#585570;">
							...
							<div show-on-laptop>
							    <md-content>
							    	<md-list class="p-0" flex>
							    		<md-list-item md-on-select="logItem" class="md-1-line" md-auto-select="options.autoSelect"  ng-repeat="empresa in dadosEmpresa|filter:buscaRapida|limitTo: query.limit: (query.page -1) * query.limit" ng-click="null">
							    			<!--
							    			<div flex="5" ng-if="cliente.pe_foto != ''">
							    				<img src="imagens_empresas/<?=base64_decode($us_id)?>/cliente_fornecedor/{{cliente.pe_foto}}" class="md-avatar" alt="">
							    			</div>
							    		...
							    		-->
							    			<div flex="5"><p class="pt-2" style="color: #000; vertical-align: middle;"><b>{{empresa.em_cod}}</b></p></div>
							    			<div flex="30"><p class="pt-2" style="color: #000; vertical-align: middle;"><b>{{empresa.em_razao | limitTo:30}}{{empresa.em_razao.length >= 30 ? '...' : ''}}</b></p></div>
							    			<div flex="30">{{empresa.em_fanta | limitTo:30}}{{empresa.em_fanta.length >= 30 ? '...' : ''}}</div>
							    			<div flex="15">{{empresa.em_cid | limitTo:15}}{{empresa.em_cid.length >= 15 ? '...' : ''}}</div>
							    			<div flex="15">{{empresa.em_fone | limitTo:15}}{{empresa.em_fone.length >= 15 ? '...' : ''}}</div>
							    			<div flex="5">
		...
		<?php }?>
							...
							<div show-on-tablet>
							    <md-content>
							    	<md-list class="p-0" flex>
							    		<md-list-item class="md-2-line" ng-repeat="empresa in dadosEmpresa" ng-click="null">
							    			<img src="data:image/jpeg;base64,{{empresa.em_logo}}" class="md-avatar"/>
							    			<div class="md-list-item-text" layout="column">
							    				<h3><b>{{empresa.em_cod}} - {{empresa.em_razao | limitTo:50}}{{empresa.em_razao.length >= 50 ? '...' : ''}}</b> - {{empresa.em_fanta | limitTo:50}}{{empresa.em_fanta.length >= 50 ? '...' : ''}}</h3>
							    				<h3>{{empresa.em_end | limitTo:50}}{{empresa.em_end.length >= 50 ? '...' :''}}, {{empresa.em_cid | limitTo:30}}{{empresa.em_cid.length >= 30 ? '...' : ''}} - {{empresa.em_fone}}</h3>
							    			</div>
				    		 				<md-fab-speed-dial ng-hide="hidden" md-direction="left" md-open="isOpen" class="md-scale md-fab-top-right" ng-mouseenter="isOpen=true" ng-mouseleave="isOpen=false">
				    		 				 	<md-fab-trigger>
								                	<md-button aria-label="menu" class="md-warn">
    ...
    <script  id="INLINE_PEN_JS_ID">
	    ...
	    angular.module("ZMPro").controller("ZMProCtrl", function ($scope, $http, $timeout, $mdSidenav, $mdEditDialog, $log) {
				...
				$http({
					method: 'POST',
					 headers: {
			          ...
			          url: $scope.urlBase+'SalvaCad.php?editarCadastrar='+$scope.cadastrar_alterar+'&e=+<?=$empresa?>&eA=<?=$empresa_acesso?>&cnpn_matriz=<?=$cnpj_matriz?>&ramo=<?=$em_ramo?>&us=<?=$us?>'
				}).then(function onSuccess(response){
					dadosEmpresa();
			...
			$scope.excluirEmpresa = function(em_cod,em_cnpj){
				$http({
					method: 'GET',
					url: $scope.urlBase+'excluirEmpresa.php?id='+em_cod+'&us=<?=$us?>&cnpj='+em_cnpj+'&e=<?=$empresa?>&eA=<?=$empresa_acesso?>'
				}).then(function onSuccess(response){
					dadosEmpresa();
